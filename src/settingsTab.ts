		...
		new Setting(containerEl)
			...
			.addDropdown((drop) =>
				drop
					...
					.onChange(async (value) => {
						this.plugin.settings.reviewOrder = value as "due" | "random";
						await this.plugin.saveSettings();
					})
			);
			...
			.addText((text) =>
				text
					...
					.onChange(async (value) => {
						...
						this.plugin.settings.dailyLimit = isNaN(n) || n < 0 ? 0 : n;
						await this.plugin.saveSettings();
					})
			);
		...
		new Setting(containerEl)
			...
			.addText((text) =>
				text
					...
					.onChange(async (value) => {
						const n = parseInt(value, 10);
						this.plugin.settings.newCardsPerSession = isNaN(n) || n < 0 ? 20 : n;
						await this.plugin.saveSettings();
					})
			);
		...
		new Setting(containerEl)
			...
			.addText((text) =>
				text
					...
					.onChange(async (value) => {
						const n = parseInt(value, 10);
						this.plugin.settings.leechThreshold = isNaN(n) || n < 0 ? 0 : n;
						await this.plugin.saveSettings();
					})
			);

		new Setting(containerEl).setName("AI quiz - bring your own key").setHeading();
